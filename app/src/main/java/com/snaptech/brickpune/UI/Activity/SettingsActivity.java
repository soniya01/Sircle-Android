...
import com.snaptech.brickpune.UI.Adapter.NotificationsGroupAdapter;

import com.snaptech.brickpune.UI.Model.NotificationGroups;
import com.snaptech.brickpune.Utility.AppError;
import com.snaptech.brickpune.Utility.Constants;
import com.snaptech.brickpune.WebService.GroupResponse;
                    ...
                    if (response != null) {
                        if (response.getStatus() == 200){
                            if (response.getData().getGroups().size() > 0){
                                System.out.println("Inside from server , size is "+response.getData().getGroups().size());
                                SettingsActivity.this.notificationGroupList.addAll(response.getData().getGroups());
                                notificationsGroupAdapter.notifyDataSetChanged();
                            }
